...
const CarouselPrevious = React.forwardRef(({ className, variant = "", size = "icon", ...props }, ref) => {
  ...
  return (
    (<Button
      ...
      variant={variant}
      size={size}
      className={cn("absolute  h-10 w-10 rounded-full bg-[#5490CB]", orientation === "horizontal"
        ? "-left-4 top-1/3 -translate-y-1/2"
        : "-top-12 left-1/2 -translate-x-1/2 rotate-90", className)}
      disabled={!canScrollPrev}
      onClick={scrollPrev}
      {...props}>
      <RiArrowLeftWideFill className="text-white font-bold text-xl" />
...
const CarouselNext = React.forwardRef(({ className, variant = "", size = "icon", ...props }, ref) => {
  ...
  return (
    (<Button
      ...
      onClick={scrollNext}
      {...props}>
      <RiArrowRightWideFill className="text-white font-bold text-xl" />
